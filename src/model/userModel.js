...
const { NotFoundError, UnauthorizedError } = require("./../../lib/appErrors");
...
const userSchema = new mongoose.Schema({
  ...
  phone: {
    ...
    validate(value) {
      if (value.length < 10) {
        throw new UnauthorizedError("Number must be greater than 10 digits");
      }
    },
  },
  email: {
    ...
    validate(value) {
      if (!validator.isEmail(value)) {
        throw new UnauthorizedError(
          "This is not an email, please provide a valide email"
        );
      }
    },
  },
...
userSchema.statics.findUserByCredentials = async (email, password) => {
  ...
  if (!isMatch)
    throw new UnauthorizedError("user with password does not match");
